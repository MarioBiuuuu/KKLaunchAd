...
typedef NS_OPTIONS(NSUInteger, MBLaunchAdImageOptions) {
    /** 有缓存,读取缓存,不重新下载,没缓存先下载,并缓存 */
    MBLaunchAdImageDefault = 1 << 0,
    /** 只下载,不缓存 */
    MBLaunchAdImageOnlyLoad = 1 << 1,
    /** 先读缓存,再下载刷新图片和缓存 */
    MBLaunchAdImageRefreshCached = 1 << 2 ,
    /** 后台缓存本次不显示,缓存OK后下次再显示(建议使用这种方式)*/
    MBLaunchAdImageCacheInBackground = 1 << 3
};
...
@interface MBLaunchAdImageManager : NSObject
